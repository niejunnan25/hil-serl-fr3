...
2026-09-21。功能已接入隔离分支 `codex/robometer-episode-gap`，默认仍为 `reward_mode=sparse`。本文描述实际代码行为。CPU、模拟设备、实际本地 HTTP/Agentlace 通信和原生 replay 采样已有测试；尚未加载真实 4B 权重或启动真机训练，7 秒计奖是调度示例，不是实测吞吐。
...
| 双方就绪 | 验证复位成功，再等待/确认入库并释放屏障；重新读取最新图像和状态，开始下一条 | 收到释放确认，结束本条工作 | 看到健康 Actor 开始下一条后恢复更新 |

这里的并行是：机器人复位在 Actor 主线程，模型推理与提交在独立 worker；worker 不调用机器人 API。采集与 Learner 之间仍保持原有的异步策略发布方式。

```text
示例：共同起点，复位 5 秒，计奖＋保存＋提交 7 秒
时间          0              5      7
机器人        [复位与验证      ][等2秒] → 下一条采集
...
额外等待结束后，在第一步动作前重新读取机器人状态和相机图像，经过与正常输入相同的相对坐标、欧拉角、状态展开和图像堆叠变换。保留原来验证通过的复位坐标原点，更新当前末端坐标变换；这次只读刷新不下发动作、不再次复位、不增加训练步数，也不运行成功分类器。
...
- 回归测试结论及已复现的基线问题见 [验证报告](robometer-episode-gap-verification.md)，不能把模拟设备/模型结果当成真实 4B 模型的性能结论。
