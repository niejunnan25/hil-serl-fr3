...
# eye ~0.85 m away, target at the parts near z=0.03. Pulled the eye slightly
# back from the original (0.6,0.6,0.5) so both parts (spread +/-0.05 m in Y)
# are comfortably inside the frame even with the wide 20.955 mm aperture.
CAM_EYE = (0.55, 0.55, 0.45)
...
PLUG_TRANSLATION = (0.0, 0.05, 0.03)
SOCKET_TRANSLATION = (0.0, -0.05, 0.015)

# Lighting tuned per droid lighting.py (measured on this exact machine):
# dome ~900-1100 ambient floor, sun ~1400-1600 directional. Keep at the high
# end because the parts are tiny on a large ground plane.
DOME_INTENSITY = 1000.0
DOME_COLOR = (0.9, 0.9, 0.9)
SUN_INTENSITY = 1400.0
...
def main() -> int:
    ...
    # -----------------------------------------------------------------------
    # 2a. ground plane (tutorial idiom: build cfg, then call cfg.func).
    try:
        ground_cfg = sim_utils.GroundPlaneCfg()
        ground_cfg.func("/World/defaultGroundPlane", ground_cfg)
        print("[viewer] ground=OK /World/defaultGroundPlane", flush=True)
    ...
    spawned = []
    plug_specs = [
        ("/World/Plug", cli_args.plug_usd, PLUG_TRANSLATION),
        ("/World/Socket", cli_args.socket_usd, SOCKET_TRANSLATION),
    ]
    for prim_path, usd_path, translation in plug_specs:
        try:
            ...
            print(f"[viewer] usd=OK {prim_path} <- {usd_path} "
                  f"translation={translation}", flush=True)
        except Exception as exc:
    ...
    # 6) Clean exit. os._exit(0) avoids Kit teardown hangs in headless; the PNG
    #    is already saved above. Fall back to simulation_app.close() first for
