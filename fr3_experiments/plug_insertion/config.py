...
class PlugEnv(USBEnv):
    """Plug insertion env scaffold.

    Gate 1 keeps USB insertion behavior as the upstream reference and only
    separates the local namespace. Fixture-specific reset/step behavior belongs
    to later live-gated phases.
    """


class EnvConfig(DefaultEnvConfig):
    SERVER_URL: str = "http://127.0.0.1:5017/"
    REALSENSE_CAMERAS = {
        "wrist_1": {
...
class TrainConfig(DefaultTrainingConfig):
    ...
    def get_environment(self, fake_env=False, save_video=False, classifier=False):
        env = PlugEnv(fake_env=fake_env, save_video=save_video, config=EnvConfig())
        if not fake_env:
